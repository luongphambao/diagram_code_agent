...
/** Ported from components/PdfReportApproval.tsx (plan §F). */
export default function PdfReportCard({ args, status, respond }: GateCardProps) {
  ...
  const title = (typeof args.title === "string" && args.title.trim()) || "Architecture Blueprint";
  const subtitle = (typeof args.subtitle === "string" && args.subtitle.trim()) || "Architecture Report";
  ...
  if (decided && status === ToolCallStatus.Executing) {
    return (
      <GateFrame label="PDF Report" tone="export" status={status}>
        <p className="text-xs text-muted">{mode === "feedback" ? "Feedback sent — updating report settings…" : "Approved — generating PDF…"}</p>
      </GateFrame>
    );
  }

  return (
    <GateFrame label="PDF Report" tone="export" status={status} badge={<Chip variant="info" numeric>{sections.length} sections</Chip>}>
      {mode === "idle" ? (
        <>
          <p className="text-sm text-secondary">{typeof args.question === "string" ? args.question : "Generate the PDF report?"}</p>
          ...
          {missingSections.length > 0 && (
            <div className="rounded-sm border border-warn/30 bg-warn/10 px-3 py-2.5">
              ...
              <p className="mt-1.5 text-2xs text-warn-text">
        ...
        <>
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs text-secondary">What should change in the report?</p>
          </div>
          <textarea
