...
export default function MeetingSlotPicker({ interrupt, onResolve, disabled = false }: MeetingSlotPickerProps) {
    ...
    <div className="flex w-full flex-col overflow-hidden rounded-2xl border border-violet-500/20 bg-violet-950/15">
      ...
      <div className="flex items-center gap-2.5 border-b border-white/8 bg-white/4 px-4 py-3">
        <div className="flex h-6 w-6 items-center justify-center rounded-full bg-violet-500/20">
          <svg className="h-3.5 w-3.5 text-violet-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div className="flex flex-col gap-3 px-4 py-4">
        ...
        {context && (
          <p className="text-[11px] italic text-slate-500">{context}</p>
        )}

        {/* Slot list */}
        {slots.length === 0 ? (
          <div className="rounded-xl border border-white/8 bg-white/4 px-4 py-4 text-center">
            <p className="text-xs text-slate-500">No available slots found. Try adjusting the date range.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-1.5">
            {(slots as SelectedSlot[]).map((slot, i) => (
              ...
              >
                <div>
                  <p className={`text-xs font-semibold ${picked === i ? "text-violet-200" : "text-slate-200"}`}>
                    {slot.display_day}
                  </p>
                  <p className={`mt-0.5 text-[11px] ${picked === i ? "text-violet-300" : "text-slate-400"}`}>
                    {slot.display_time}
                    {timezone ? <span className="ml-1.5 opacity-60">{timezone}</span> : null}
                  </p>
                </div>
                {/* Radio indicator */}
                <div className={`flex h-4 w-4 flex-shrink-0 items-center justify-center rounded-full border-2 transition-colors ${
