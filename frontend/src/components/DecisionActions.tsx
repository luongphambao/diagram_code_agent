...
export default function DecisionActions({
  allowedDecisions, assumptions = [], disabled = false, approveLabel = "Approve",
  onApprove, onReject, onDecision,
  ...
  const reset = () => { setOpen(null); setText(""); setOwner(""); setPicked([]); };

  const submit = (action: DecisionAction) => {
    if (action === "reject") { if (!text.trim()) return; onReject(text.trim()); reset(); return; }
    if (action === "approve_with_assumptions") {
      if (!picked.length) return;
      onDecision({ action, approved: true, assumption_ids: picked, comment: text.trim() || undefined });
      reset(); return;
    }
    ...
    return (
      <div className="flex flex-col gap-2.5 rounded-xl border border-white/10 bg-black/20 px-3 py-3">
        <div className="flex items-center gap-2">
          <button onClick={reset} className="text-slate-600 hover:text-slate-400">← Back</button>
              ...
              {assumptions.map((a) => (
                <label key={a.id} className="flex items-start gap-2 text-[11px] text-slate-300">
                  <input type="checkbox" checked={picked.includes(a.id)} onChange={() => togglePick(a.id)}
                    className="mt-0.5" disabled={disabled} />
                  <span><span className="text-slate-500">{a.id}</span> · {a.statement}</span>
                </label>
              ))}
            </div>
          ) : (
        ...
        {open === "accept_risk" && (
          <input value={owner} onChange={(e) => setOwner(e.target.value)} disabled={disabled}
        ...
        {open !== "approve_with_assumptions" && (
          <textarea value={text} onChange={(e) => setText(e.target.value)} rows={2} disabled={disabled} autoFocus
            placeholder={
              open === "accept_risk" ? "Describe the risk you are accepting…"
                : open === "request_evidence" ? "Which claim needs a source?"
                : open === "request_alternative" ? "What alternative / constraint to compare? (optional)"
        ...
        <button onClick={() => submit(open)} disabled={disabled}
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      {secondary.map((a) => (
        <button key={a} onClick={() => setOpen(a)} disabled={disabled}
