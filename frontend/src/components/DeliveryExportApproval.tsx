...
export default function DeliveryExportApproval({ interrupt, onResolve, onDecision, disabled = false }: DeliveryExportApprovalProps) {
  ...
  const approve = () => { setDecided(true); setDecision("approved"); onResolve(true); };
  const cancel = () => { setDecided(true); setDecision("rejected"); onResolve(false); };
    ...
    <div className="flex w-full flex-col overflow-hidden rounded-2xl border border-indigo-500/20 bg-indigo-950/15">
      ...
      <div className="flex items-center gap-2.5 border-b border-white/8 bg-white/4 px-4 py-3">
        <div className="flex h-6 w-6 items-center justify-center rounded-full bg-indigo-500/20">
          <svg className="h-3.5 w-3.5 text-indigo-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      {decided ? (
        <div className="px-4 py-3">
          <p className="text-xs text-slate-500">
            {decision === "approved"
              ? (live ? `Syncing work items to ${sys}…` : `Building ${sys} preview…`)
              : "Export cancelled."}
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-3 px-4 py-4">
          ...
          <div className="rounded-xl border border-white/8 bg-white/3 px-3 py-2.5">
            <div className="flex items-start gap-2">
              <svg className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-indigo-400/70" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
