...
export default function ActivityRow({ entry }: { entry: LogEntry }) {
  ...
  return (
    <div className={`rounded-md border ${isError ? "border-red-500/20 bg-red-500/5" : "border-white/5 bg-white/3"}`}>
      <button
      ...
      {expanded && hasDetail && (
        <pre className={`whitespace-pre-wrap border-t border-white/5 px-3 py-2 font-mono text-[10px] leading-relaxed ${isError ? "text-red-400/80" : "text-slate-500"}`}>
          {detail}
        </pre>
      )}
    </div>
