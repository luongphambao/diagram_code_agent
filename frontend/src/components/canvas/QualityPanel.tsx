      ...
      {quality && (
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            <div
              className={`flex h-16 w-16 flex-col items-center justify-center rounded-2xl border ${GRADE_CLS[grade] || GRADE_CLS.F}`}
            >
              <span className="text-2xl font-bold leading-none">{grade}</span>
              <span className="mt-0.5 text-[10px] opacity-80">
                {(quality.quality_score ?? 0).toFixed(0)}/100
              </span>
            </div>
            ...
            <Section title="Findings">
              <div className="flex flex-wrap gap-1.5">
                ...
                {dimEntries.map(([dim, n]) => (
                  <span
                    key={dim}
                    className="rounded border border-white/8 bg-white/5 px-2 py-0.5 text-[11px] text-slate-400"
                  >
                    {dim.replace(/_/g, " ")}: {n}
                  </span>
                ))}
                ...
                {compliance.controls.map((c) => (
                  <tr key={c.id} className="border-b border-white/5">
                    ...
                    <td className="py-1 text-right">
                      <span
                        className={`text-[10px] ${c.grounded ? "text-emerald-400" : "text-red-400/80"}`}
                      >
      ...
      {drift && ds && (
        <Section title="Reality drift">
          <div className="mb-2 flex flex-wrap gap-1.5">
            <span className="rounded-full border border-emerald-500/25 bg-emerald-500/10 px-2.5 py-0.5 text-[11px] text-emerald-300">
              {ds.matched ?? 0} matched
            </span>
            <span className="rounded-full border border-amber-500/25 bg-amber-500/10 px-2.5 py-0.5 text-[11px] text-amber-300">
              {ds.in_design_not_in_reality ?? 0} design-only
            </span>
            <span className="rounded-full border border-orange-500/25 bg-orange-500/10 px-2.5 py-0.5 text-[11px] text-orange-300">
              {ds.in_reality_not_in_design ?? 0} reality-only
            </span>
          </div>
