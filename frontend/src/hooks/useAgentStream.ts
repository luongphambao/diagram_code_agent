  ...
  const runAgent = useCallback(async (messages: WireMessage[]) => {
    ...
    const subscriber: AgentSubscriber = {
      onTextMessageStartEvent: ({ event }) => {
        ...
        setChatMessages((prev) => [...prev, { id: event.messageId, role: "assistant", content: "" }]);
      },
      ...
      onTextMessageEndEvent: ({ event }) => {
        ...
        wireMessagesRef.current = [...wireMessagesRef.current, { id: event.messageId, role: "assistant", content }];
      },

      onToolCallEndEvent: ({ event, toolCallArgs }) => {
        lastTcIdRef.current = event.toolCallId;
        setPendingInterrupt({ toolCallId: event.toolCallId, data: toolCallArgs as PendingInterrupt["data"] });
      },
      ...
      onCustomEvent: ({ event, state }) => {
        ...
        if (phase === "start") {
          ...
          logs = [...prevLogs, { t: 0, type: "tool_start", tool, label, input: payload.detail ?? "", subagent }];
        } else {
          ...
          logs = [...prevLogs, {
            t: 0, type: "tool_end", tool, label, output: payload.detail ?? "",
            error: payload.ok === false ? (payload.detail || "Tool returned an error") : undefined,
            elapsed_s, subagent, ok: payload.ok,
          }];
    ...
    threadIdRef, userRoleRef, uploadedFileIds, agentStateRef, setAgentState, setIsRunning, setActivity,
    setActiveSubagent, setError, setPendingInterrupt, setChatMessages, wireMessagesRef, lastTcIdRef,
  ]);
