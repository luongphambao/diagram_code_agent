...
def test_comment_append_and_read(tmp_path: Path):
    rec = new_comment_record(
        "Is 500 RPS realistic?",
        seq=next_seq(tmp_path),
        anchor_entity_id="REQ-3",
        author="alice",
        role="architect",
    )
    assert rec.id == "CMT-1"
...
def test_resolve_comment(tmp_path: Path):
    ...
    updated = resolve_comment(
        "CMT-1", resolved_by="bob", resolved_at="2026-06-30T00:00:00Z", workspace=tmp_path
    )
...
def test_can_approve_policy():
    ...
    # CRITICAL-1 fix: empty/unknown role is now DENIED on a restricted gate —
    # the old permissive default assumed the frontend never sent a role, which
    # is no longer true (frontend/src/App.tsx's userRole selector, default
    # "lead", is sent on every /agui request) and only ever helped a caller
    # that skipped the frontend/its role selector entirely.
    assert can_approve("", "propose_blueprint") is False
    # a gate with no role restriction is open to anyone (unaffected by the
    # change above — draft/skeleton gates like propose_deck_plan are
    # deliberately left unrestricted, see can_approve's docstring)
    assert can_approve("client", "propose_deck_plan") is True
    assert can_approve("", "propose_deck_plan") is True


def test_decision_record_carries_role():
    rec = DecisionRecord(
        id="DEC-h1",
        gate="propose_blueprint",
        action="accept_risk",
        approver="a@b.com",
...
def _seed_model(ws: Path) -> None:
    model = SolutionModel(
        decisions=[
            Decision(
                id="DEC-1",
                title="Use managed Kubernetes",
                options=[
                    DecisionOption(id="o1", title="EKS", trade_offs="ops simplicity"),
                    DecisionOption(id="o2", title="self-managed", trade_offs="cheaper, more ops"),
                ],
                selected_option_id="o1",
                rationale="Lower ops burden",
                status="approved",
                approver="arch@x.com",
                evidence_ids=["EVD-1"],
                risk_ids=["RISK-2"],
...
def test_adr_pack_includes_approval_timeline(tmp_path: Path):
    ...
    rec = new_decision_record(
        ...
        revision=2,
        comment="accepted single-region risk",
    )
