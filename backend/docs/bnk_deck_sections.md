...
[`backend/src/domain/deck/deck_sections.py`](../src/domain/deck/deck_sections.py) (`SECTION_CONTENT_CONTRACTS`) —
...
3. `tools/analysis/reporting_gates.py`: thêm tool `capture_business_narrative` (ghi
...
System** (banking, AWS). Dùng để chứng minh resolver [`deck_resolver.py`](../src/domain/deck/deck_resolver.py)
