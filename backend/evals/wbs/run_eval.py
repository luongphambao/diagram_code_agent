"""WBS eval runner — deterministic, no agent/LLM, CI-safe.

    cd backend
    uv run python -m evals.wbs.run_eval
    uv run python -m evals.wbs.run_eval --gate
    uv run python -m evals.wbs.run_eval --update-baseline
"""
...
from evals.wbs.judge import METRIC_KEYS, score_wbs  # noqa: E402
...
_COLUMNS = [
    ("case_id", "case"),
    ("scores.micro", "micro"),
    ("scores.coverage_ok", "cover"),
    ("scores.dependency_ok", "deps"),
    ("scores.critical_path_ok", "cpath"),
    ("scores.rollup_ok", "rollup"),
]


def _run_one(case: dict) -> dict:
    return {"case_id": case.get("id", "unknown"), "scores": score_wbs(case)}
...
def main() -> None:
    ...
    if not cases:
        print(f"No WBS cases in {_DATASET}")
        sys.exit(1)
