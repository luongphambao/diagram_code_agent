...
The ``_current_workspace`` ContextVar below is defined in exactly ONE place —
this module. ``src/backends.py`` (kept as a re-export shim after this module's
introduction) re-exports the SAME object; it must never re-instantiate its own
``contextvars.ContextVar(...)``, or per-thread workspace binding silently splits
into two independent vars (§4.10 multi-tenancy — see test_workspace_isolation.py).
"""
...
# backend/src/runtime/backends.py -> parents[2] == backend/
