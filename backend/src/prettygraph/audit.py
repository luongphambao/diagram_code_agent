...
def audit_layout(dot_path: str, png_path: str | None = None) -> str:
    ...
    try:
        g = json.loads(
            run_graphviz(["dot", "-Tjson", dot_path], capture_output=True, text=True, check=True).stdout
        )
    ...
    lines = [
        f"Layout audit: aspect {aspect:.2f}:1"
        + (
            " — OK"
            if 0.55 <= aspect <= 2.1
            else " — TOO WIDE for the slide panel, fold cross-cutting tiers into a 2nd row (≤5 columns)"
            if aspect > 2.1
            else " — very tall, consider direction='LR'"
        ),
        f"  clusters: {n_clusters}",
    ]
    if long_labeled:
        lines.append(
            "  STRAND RISK — these labeled edges span far; their labels "
    ...
    if len(node_pts) >= 8 and central_pts == 0:
        lines.append(
            "  SPARSE CENTER — no nodes in the central canvas. This often "
    ...
    if (
        ((0, 0) in occupied or (1, 0) in occupied)
        and ((2, 1) in occupied or (2, 2) in occupied)
        and (1, 1) not in occupied
    ):
        lines.append(
            "  L-SHAPE WARNING — nodes are packed along the bottom and "
    ...
    if edge_count and dashed_edges > max(4, edge_count * 0.35):
        lines.append(
            f"  SIDE-CHANNEL FANOUT — {dashed_edges}/{edge_count} edges "
...
def _audit_text_fit(dot_path: str) -> list[str]:
    ...
    try:
        sidecar = Path(dot_path).with_name(Path(dot_path).name.replace(".dot", ".nodes.json"))
    ...
    for meta in side.get("nodes", {}).values():
        ...
        need = (
            max(
                _est_text_w(meta.get("label") or "", title_s, bold=True),
                _est_text_w(meta.get("sublabel") or "", sub_s),
            )
            + icon_w
            + 24
        )
        if need > nw:
            ...
            over.append(
                f'"{meta.get("label")}" needs ~{need:.0f}pt > node_width {nw}pt (<= {fits} chars fits)'
            )
    if not over:
        return []
    return [
        "  TEXT OVERFLOW — these cards were auto-widened so text stays "
