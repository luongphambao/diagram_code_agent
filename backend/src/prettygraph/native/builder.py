...
class Diagram:
    ...
    def __init__(self, type="pipeline", *, title="", page=(2000, 1200), contract="scaffold", flat=False):
    ...
    def box(
        ...
        *,
        ...
        style = (
            f"rounded={1 if round else 0};whiteSpace=wrap;html=1;fillColor={fill};"
    ...
    def card(
        ...
        *,
        ...
        sh = self._put(
            ...
            w,
            h,
            "rounded=1;arcSize=12;whiteSpace=wrap;html=1;fillColor=#1F2A37;opacity=12;strokeColor=none;",
            "",
            z=Z_SHADOW,
        )
        sh["ob"] = False
        ...
        if sub:
            label += f'<br><font style="font-size: 10px" color="#647687">{_esc(sub)}</font>'
        ...
        if accent:
            ...
            ac = self._put(
                ...
                max(10, w - 16),
                4,
                f"rounded=1;arcSize=60;html=1;fillColor={accent};strokeColor=none;",
                "",
                z=Z_FORE,
            )
            ac["ob"] = False
        if has_icon:
            if image_data_uri:
                ...
                icon_style = s["style"]
            ir = self._put(
                f"{id}__ic", id, round(x + 10), round(y + (h - ic) / 2), ic, ic, icon_style, "", z=Z_FORE
            )
            ir["ob"] = False
        return r
    ...
    def legend(self, entries, xy, *, id="__legend") -> dict | None:
        ...
        for i, (label, color, dashed) in enumerate(entries):
            ...
            lb = self._put(
                ...
                w - sw - pad * 2 - 8,
                row_h,
                f"text;html=1;align=left;verticalAlign=middle;fontSize=11;fontColor={THEME.font_color};",
                label,
                z=Z_CHROME,
            )
            lb["ob"] = False
    ...
    def pill(
        ...
        *,
        ...
        style = (
            f"rounded=1;arcSize={arc};html=1;whiteSpace=wrap;fillColor={fill};"
    ...
    def note_card(
        self, id, xy, wh, title, lines=(), *, fill=None, stroke="#D0D5DD", fs=None, parent="1"
    ) -> dict:
    ...
    def legend_band(
        self, id, xy, w, entries, *, scope_note="", metadata="", title="CONNECTOR SEMANTICS & SCOPE", h=None
    ) -> dict:
        ...
        for i, (label, color, dashed) in enumerate(entries or []):
            ...
            lb = self._put(
                ...
                f"fontSize={RT.TYPE_SCALE['legend']};",
                label,
                z=Z_CHROME,
            )
            lb["ob"] = False
        ...
        if scope_note:
            ...
            if sw >= 200:
                sn = self._put(
                    ...
                    f"spacing=10;fontFamily={RT.FONT};"
                    f"fontColor={RT.INK['body']};fontSize=9.5;",
                    f"<b>Scope</b><br>{_esc(scope_note)}",
                    z=Z_CHROME,
                )
                sn["ob"] = False
        return band

    def group(self, id, gname, xy, wh, label="", *, parent="1", fill=None, stroke=None) -> dict:
        ...
        if gname == "group_subnet":
            ...
            stroke = stroke or (THEME.subnet_private_stroke if priv else THEME.subnet_public_stroke)
    ...
    def cluster_box(
        ...
        *,
        ...
        if icon:
            ...
            self._put(
                f"{id}_icon", "boundaries", x + 1, y + 1, icon_size, icon_size, s["style"], "", z=Z_FORE
            )
        return self.R[id]
    ...
    def link(self, src, tgt, label="", **opts) -> "Diagram":
        for node in (src, tgt):
            if node not in self.R and node not in self.phantoms:
                raise ValueError(f'link(): unknown node id "{node}" — not built and not a phantom.')
    ...
    def _build_edges(self) -> None:
        ...
        self.edge_specs = [e for e in self.edge_specs if e["src"] in self.R and e["tgt"] in self.R]
    ...
    def to_xml(self) -> str:
        ...
        bounds_layer = (
            ...
            if 'parent="boundaries"' in cells_xml
            else ""
        )
        return (
    ...
    def mxfile(self, name="Diagram", extra_pages: list[tuple[str, str]] | None = None) -> str:
